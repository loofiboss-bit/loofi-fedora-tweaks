...
| --- | --- | --- | --- |
| v30.1.0 | Personalize | ACTIVE | [architecture](.workflow/specs/arch-v30.1.0.md), [tasks](.workflow/specs/tasks-v30.1.0.md), [candidate notes](docs/releases/RELEASE-NOTES-v30.1.0.md) |
...
## [ACTIVE] v30.1.0 "Personalize" — Direct Fedora Tweaks

Home, Apps, Tweaks, Health, and Updates are the five primary destinations.
Tweaks offers eight capability-scoped settings with current values and direct,
verified application. Health owns storage trim, package cache cleanup, and
symptom-led repair reviews. Older route IDs and saved runs remain readable.
This is local implementation work; no tag, publication, or host installation
is authorized. See the [architecture](.workflow/specs/arch-v30.1.0.md),
[tasks](.workflow/specs/tasks-v30.1.0.md), and
[candidate notes](docs/releases/RELEASE-NOTES-v30.1.0.md).

## [LOCAL CANDIDATE] v30.0.1 "Steady" — Stability and Measured Startup Cost

v30.0.1 tightens the existing Utility workflows without adding user features.
It makes asynchronous operation results agree with durable Action Center state,
proves per-item Install and stop-on-error Tune results, keeps Update sources
independent, and realizes Install, Tune, Fix, and Update only on first use.

This is a local release candidate. Installation, publication, tagging, and
host mutation are outside its scope. The historical `v30.0.0` tag and its
"Distribution & Reliability" lineage remain unchanged. The performance
acceptance gate compares ten clean offscreen starts and requires at least a
10% median improvement in startup time or RSS while limiting regression in the
other metric to 15%. Local qualification passed. Remaining physical-gate
statuses and evidence are recorded in the v30.0.1 qualification report and
candidate tasks.

See the [v30.0.1 plan](docs/plans/LOOFI_FEDORA_TWEAKS_V30_PLAN.md),
[architecture](.workflow/specs/arch-v30.0.1.md), and
[release tasks](.workflow/specs/tasks-v30.0.1.md).

## [DONE] v29.0.1 "Utility" — Curated Fedora Utility
...
read-only and preserve their history. Exact public publication evidence is
recorded in the [v28.0.3 release report](docs/reports/V28.0.3_RELEASE_PUBLICATION.md).
...
Public release for the battery-service review fixes merged from PR #40.
...
public wiki readback are recorded in the
[v28.0.2 public release evidence](docs/reports/V28.0.2_RELEASE_PUBLICATION.md);
physical and manual qualification remains a separate evidence track.
...
Public release complete: Loofi Fedora Tweaks is a desktop-neutral Fedora
Maintenance Core with five destinations, immutable PlatformProfile, native
authorization handoffs, and strict Action Center execution. The historical
v27.0.0 Marketplace Enhancement tag remains unchanged and v27.0.1 is the
unique Core release identity. See the [public release evidence](docs/reports/V27_RELEASE_PUBLICATION.md)
for the exact tag, assets, attestations, COPR package, and wiki readback.
...
86.94% maintained-surface coverage against the 85% blocking gate. The planned
repository-wide 90% target and physical/manual Fedora qualification are
explicitly deferred beyond this release under the authorized release decision.

## [DONE] v26.0.3 "Everyday" — Daily Maintenance

**Status:** public release complete. Published on 2026-09-13 via run 34754130936
and COPR build 10981363. Explicit System/Flatpak/Firmware overview, saved-run
follow-up, plain-language actions, and rootless qualification. See the
[plan](docs/plans/LOOFI_FEDORA_TWEAKS_V26_PLAN.md),
[tasks](.workflow/specs/tasks-v26.0.3.md),
[architecture](.workflow/specs/arch-v26.0.3.md),
[qualification](docs/reports/V26_RELEASE_QUALIFICATION.md), and
[public evidence](docs/reports/V26_RELEASE_PUBLICATION.md).
...
`request → plan → fresh preflight → optional compact confirmation → execute →
independent verify → typed outcome`.

The candidate includes fail-closed catalog eligibility, versioned Safety &
Execution settings, the `loofi run ACTION_ID` CLI, the same asynchronous GUI
path for high-traffic actions, Home pending/reboot/last-verified/recovery
projections, and Activity & Recovery 2.0 filters, evidence, and redacted export.
Unknown, incomplete, high-risk, manual-only, unsupported, unverifiable, and
future-schema requests remain review-only or blocked. Physical and public
qualification is reported separately and is never inferred from rootless or
offscreen evidence.

## [DONE] v24.0.0 "Flow" — Coherent Desktop Workflows
...
and audible Orca remain unverified. Annotated tag lineage, CI and CodeQL, eight
GitHub assets and attestations, checksums, SBOM, provenance, COPR build
`10838092`, signed EVR `1:24.0.0-1.fc44`, a clean disposable Fedora 44 install,
and the public wiki passed independent readback.
...
The exact v23.1.0 release passed the full local 7,048-test gate, 86.36%
coverage, lint, typecheck, architecture, stabilization, product contract, and
real Fedora 44 KDE Plasma Wayland screenshot capture at 140% scale. Annotated
tag lineage, CI and CodeQL, eight GitHub assets, checksums, attestations, COPR
build `10802361`, signed EVR `1:23.1.0-1.fc44`, a clean Fedora 44 installation,
and public docs and wiki all passed independent readback. Fresh Atomic and
manual keyboard and audible Orca qualification remain explicitly unverified
under the authorized physical-gate skip.
...
API, and Support Bundle v13. Home, search, page construction, and API GET
requests never collect. No new database, HTTP collection, plan creation, or
execution authority has been added. The
