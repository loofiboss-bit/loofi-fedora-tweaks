...
class TestGetGpuMode(unittest.TestCase):
    @patch("services.hardware.hardware.cached_which", return_value="/usr/bin/envycontrol")
...
class TestGetAvailableGpuTools(unittest.TestCase):
    @patch("services.hardware.hardware.cached_which", side_effect=lambda x: "/usr/bin/" + x if x == "envycontrol" else None)
...
class TestGetAiCapabilities(unittest.TestCase):
    ...
    @patch("subprocess.run")
    @patch("services.hardware.hardware.cached_which", side_effect=lambda x: "/usr/bin/nvidia-smi" if x == "nvidia-smi" else None)
    @patch("glob.glob", return_value=[])
    @patch("os.path.exists", return_value=False)
