...
class TestCmdAdvanced(unittest.TestCase):
    ...
    def test_advanced_bbr(self, mock_bbr, mock_print):
        _set_json(False)
    ...
    @patch("cli.main._print")
    @patch(
        "cli.main.AdvancedOps.install_gamemode",
        return_value=OperationResult(success=True, message="Gamemode installed"),
    )
    def test_advanced_gamemode(self, mock_gamemode, mock_print):
        _set_json(False)
    ...
    def test_advanced_swappiness(self, mock_swap, mock_print):
        _set_json(False)
...
class TestCmdInfo(unittest.TestCase):
    ...
    @patch("utils.operations.TweakOps.get_power_profile", return_value="balanced")
    @patch(
        "services.system.SystemManager.get_package_manager", return_value="rpm-ostree"
    )
    @patch("services.system.SystemManager.is_atomic", return_value=True)
    def test_info_json_atomic(
        self, mock_at, mock_pm, mock_prof, mock_pending, mock_json
    ):
        _set_json(True)
        r = cmd_info(_ns())
        self.assertEqual(r, 0)
    ...
    @patch("services.system.SystemManager.is_atomic", return_value=True)
    def test_info_text_atomic(
        self, mock_at, mock_pm, mock_prof, mock_pending, mock_print
    ):
        _set_json(False)
        r = cmd_info(_ns())
        self.assertEqual(r, 0)
...
class TestCmdHealth(unittest.TestCase):
    def _make_health(self, *, mem=True, cpu=True):
        ...
        if mem:
            h.memory = SimpleNamespace(
                used_human="4 GB", total_human="16 GB", percent_used=25
            )
            h.memory_status = "ok"
        else:
            h.memory = None
    ...
    @patch("utils.operations.TweakOps.get_power_profile", return_value="balanced")
    @patch(
        "services.hardware.DiskManager.check_disk_health",
        return_value=("ok", "healthy"),
    )
    ...
    @patch(
        "services.hardware.DiskManager.check_disk_health",
        return_value=("warning", "disk 80%"),
    )
    @patch("utils.monitor.SystemMonitor.get_system_health")
    def test_health_text_no_mem_no_cpu(
        self, mock_health, mock_disk, mock_prof, mock_atomic, mock_var, mock_print
    ):
        _set_json(False)
    ...
    @patch(
        "services.hardware.DiskManager.check_disk_health",
        return_value=("critical", "disk 95%"),
    )
    @patch("utils.monitor.SystemMonitor.get_system_health")
    def test_health_text_critical(
        self, mock_health, mock_disk, mock_prof, mock_atomic, mock_var, mock_print
    ):
        _set_json(False)
        h = self._make_health()
...
class TestCmdDisk(unittest.TestCase):
    ...
    def test_disk_text_details(self, mock_usage, mock_dh, mock_large, mock_print):
        ...
        mock_large.return_value = [
            SimpleNamespace(size_human="5 GB", path="/home/docs")
        ]
...
class TestCmdNetmon(unittest.TestCase):
    ...
    @patch("utils.network_monitor.NetworkMonitor.get_all_interfaces")
    def test_netmon_text_with_connections(
        self, mock_ifaces, mock_bth, mock_bw, mock_conns, mock_print
    ):
        _set_json(False)
        mock_ifaces.return_value = [self._iface()]
...
class TestCmdSupportBundle(unittest.TestCase):
    ...
    def test_support_bundle_json(self, mock_export, mock_json):
        _set_json(True)
        mock_export.return_value = SimpleNamespace(
            success=True, message="done", data="/tmp/bundle.zip"
        )
...
class TestCmdService(unittest.TestCase):
    ...
    def test_service_list(self, mock_list, mock_print):
        ...
        r = cmd_service(
            _ns(
                action="list", user=False, filter=None, search=None, name=None, lines=50
            )
        )
        self.assertEqual(r, 0)
    ...
    def test_service_logs_no_name(self, mock_print):
        ...
        r = cmd_service(
            _ns(
                action="logs", name=None, user=False, filter=None, search=None, lines=50
            )
        )
        self.assertEqual(r, 1)
...
class TestCmdPackage(unittest.TestCase):
    ...
    def test_package_list(self, mock_list, mock_print):
        ...
        r = cmd_package(
            _ns(action="list", name=None, query=None, source="all", search="", days=30)
        )
        self.assertEqual(r, 0)
    ...
    def test_package_recent(self, mock_recent, mock_print):
        ...
        r = cmd_package(
            _ns(
                action="recent", name=None, query=None, source=None, search=None, days=7
            )
        )
        self.assertEqual(r, 0)
...
class TestCmdBluetooth(unittest.TestCase):
    ...
    def test_bt_status_no_adapter(self, mock_status, mock_print):
        _set_json(False)
        mock_status.return_value = SimpleNamespace(
            adapter_name="", powered=False, discoverable=False, adapter_address=""
        )
    ...
    def test_bt_power_on(self, mock_power, mock_print):
        ...
        r = cmd_bluetooth(
            _ns(action="power-on", address=None, paired=False, timeout=10)
        )
        self.assertEqual(r, 0)
    ...
    def test_bt_power_off(self, mock_power, mock_print):
        ...
        r = cmd_bluetooth(
            _ns(action="power-off", address=None, paired=False, timeout=10)
        )
        self.assertEqual(r, 0)
    ...
    def test_bt_scan_json(self, mock_print, mock_scan, mock_json):
        _set_json(True)
        dev = SimpleNamespace(
            address="11:22:33", name="X", device_type=SimpleNamespace(value="audio")
        )
        mock_scan.return_value = [dev]
        r = cmd_bluetooth(_ns(action="scan", address=None, paired=False, timeout=5))
        self.assertEqual(r, 0)
...
class TestCmdProfile(unittest.TestCase):
    ...
    def test_profile_apply(self, mock_apply, mock_print):
        ...
        mock_apply.return_value = SimpleNamespace(
            success=True, message="applied", data={}
        )
        r = cmd_profile(
            _ns(
                action="apply",
    ...
    def test_profile_create(self, mock_capture, mock_print):
        _set_json(False)
        mock_capture.return_value = SimpleNamespace(
    ...
    def test_profile_import(self, mock_import, mock_print):
        ...
        mock_import.return_value = SimpleNamespace(
            success=True, message="imported", data={}
        )
        r = cmd_profile(
            _ns(
                action="import",
    ...
    def test_profile_import_all(self, mock_import, mock_print):
        _set_json(False)
        mock_import.return_value = SimpleNamespace(
            success=True, message="bundle imported", data={}
        )
        r = cmd_profile(
            _ns(
                action="import-all",
...
class TestCmdHealthHistory(unittest.TestCase):
    ...
    def test_hh_record(self, mock_cls, mock_print):
        _set_json(False)
        inst = MagicMock()
        inst.record_snapshot.return_value = SimpleNamespace(
    ...
    def test_hh_export_json_fmt(self, mock_cls, mock_print):
        _set_json(False)
        inst = MagicMock()
        inst.export_metrics.return_value = SimpleNamespace(
            success=True, message="exported", data={}
        )
        mock_cls.return_value = inst
    ...
    def test_hh_prune(self, mock_cls, mock_print):
        _set_json(False)
        inst = MagicMock()
        inst.prune_old_data.return_value = SimpleNamespace(
            success=True, message="pruned", data={}
        )
        mock_cls.return_value = inst
...
class TestCmdLogs(unittest.TestCase):
    ...
    def test_logs_show(self, mock_logs, mock_print):
        ...
        r = cmd_logs(
            _ns(
                action="show", unit=None, priority=None, since=None, lines=50, path=None
            )
        )
        self.assertEqual(r, 0)
