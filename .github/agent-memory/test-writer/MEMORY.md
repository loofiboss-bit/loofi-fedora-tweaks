...
## Plugin System Testing (v25.0)
- Test files: `tests/test_plugin_registry.py`, `test_plugin_loader.py`, `test_plugin_compat.py`, `test_plugin_integration.py`
- `PluginRegistry.reset()` in setup_method/teardown_method for singleton isolation
- Stub plugins: subclass `PluginInterface` directly (no PyQt6 needed); `create_widget` raises NotImplementedError
- Patch `core.plugins.loader._BUILTIN_PLUGINS` to single entries for unit tests
- Integration: mock all 26 builtins via `patch("importlib.import_module", side_effect=fn)`
- `CompatibilityDetector`: mock private methods at instance level + pre-populate `_cache` to bypass I/O
- GOTCHA: Empty `PluginRegistry` is falsy (`__len__==0`). `PluginLoader(registry=empty_reg)` silently falls through to singleton via `registry or instance()`. Tests must use a non-empty registry or the `None` keyword.
- `_read_fedora_version` uses `builtins.open` — mock with `patch("builtins.open", ...)`
- `subprocess.run` in `_check_package` — mock with `patch("subprocess.run", return_value=MagicMock(returncode=0))`
...
- Empty singleton registries are falsy — the `or` idiom in constructors silently ignores them
